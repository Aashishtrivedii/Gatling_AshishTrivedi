...
class SubsLiveCheckoutt extends Simulation {
...
//	val admin = scenario("SubsLiveCheckoutadmin").exec(Login.login,SearchandCheckout.checkout)
	val normaluser=scenario("SubsLiveCheckoutnormaluser").exec(SearchandCheckout.checkout)


	setUp(normaluser.inject(nothingFor(5),
		atOnceUsers(users =1),
		rampUsers(users = 5)during(10),
		constantUsersPerSec(rate = 14) during(20)
	))
